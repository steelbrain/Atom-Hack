...
module.exports = new function () {
    var me = this,
        ...
        handle_remote = function () {
            if (!validate())
                return;
            var dir = my_dir().replace(atom.project.path, config.remoteDir).replace(path.sep, '/');
            ssh_exec({
                cmd: 'hh_client --from atom --json "' + dir + '"',
                ssh: ssh_conn
                    ...
                    for (var i in output.errors) {
                        if (output.errors.hasOwnProperty(i)) {
                            var error = {
                                ...
                                'file': output.errors[i]['message'][0]['path'].replace(config.remoteDir, atom.project.path).replace('/', path.sep),
                                'trace': []
                            };
                            ...
                            for (var ii in output.errors[i]['message']) {
                                if (output.errors[i]['message'].hasOwnProperty(ii)) {
                                    error.trace.push({
                                        ...
                                        'file': output.errors[i]['message'][ii]['path'].replace(config.remoteDir, atom.project.path).replace('/', path.sep)
                                    });
                                }
                            }
                            errors.push(error);
