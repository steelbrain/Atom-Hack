            ...
            for (var i in errors) {
                if (errors.hasOwnProperty(i)) {
                    if (errors[i].file === current_file) {
                      ...
                      var file_path = errors[i].file;
                      if(starts_with(file_path,atom.project.path)){
                        file_path = file_path.replace(atom.project.path+path.sep,'');
                      }
                      msgPanel.append.message(errors[i].message + ' @Line# ' + errors[i].line + ' Col ' + errors[i].char_start + ' in file ' + file_path, 'text-warning');
                    }
                }
            }
            error_post();
