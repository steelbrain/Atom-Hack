...
module.exports = new function(){
  var me = this,
      ...
      handle_remote = function(){
        if(!validate())
          return;
        var dir = my_dir().replace(atom.project.path,me.config.remoteDir);
            ...
            for(var i in output.errors){
              if(output.errors.hasOwnProperty(i)){
                var error = {
                  ...
                  'file' : output.errors[i]['message'][0]['path'].replace(me.config.remoteDir,atom.project.path),
                  'trace' : []
                };
                ...
                for(var ii in output.errors[i]['message']){
                  if(output.errors[i]['message'].hasOwnProperty(ii)){
                    error.trace.push({
                      ...
                      'file' : output.errors[i]['message'][ii]['path'].replace(me.config.remoteDir,atom.project.path)
                    });
                  }
                }
                errors.push(error);
