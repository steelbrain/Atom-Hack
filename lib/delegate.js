  ...
  async exec(command: string, cwd: string, stdin: string = ''): Promise<{ stdout: string, stderr: string }> {
    if (this.config.type === 'local') {
      if (process.platform === 'win32' && this.supportBashOnWindows) {
        ...
        return await exec(`${windir}\\${process.arch === 'ia32' ? 'Sysnative' : 'System32'}\\cmd.exe`, ['/c', `bash -c ${shellEscape(['hh_client', command])}`], { cwd, stdin }, { ignoreExitCode: true })
      }
      return await exec('hh_client', [command], { cwd, stdin }, { ignoreExitCode: true })
    }
    invariant(this.ssh)
