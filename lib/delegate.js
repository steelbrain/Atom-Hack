  ...
  async exec(command: string, cwd: string, stdin: string = ''): Promise<{ stdout: string, stderr: string }> {
    if (this.config.type === 'local') {
      if (process.platform === 'win32' && this.supportBashOnWindows) {
        ...
        return await exec(`${windir}\\${process.arch === 'ia32' ? 'Sysnative' : 'System32'}\\cmd.exe`, ['/c', `bash -c '${shellEscape(['hh_client', command])}'`], { cwd, stdin, stream: 'both', ignoreExitCode: true })
      }
      return await exec('hh_client', [command], { cwd, stdin, stream: 'both', ignoreExitCode: true })
    }
    invariant(this.ssh)
    return await this.ssh.execCommand(command, { cwd: this.toRemotePath(cwd), stdin })
  }
  ...
  toLocalPath(remotePath: string): string {
    if (this.config.type === 'local') {
      if (process.platform === 'win32' && this.supportBashOnWindows) {
        ...
        return newPath.substr(0, driveLetter) + ':' + newPath.substr(driveLetter)
      }
      return remotePath
    }
