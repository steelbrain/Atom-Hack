  ...
  provideLinter: function() {
    const Linter = {
      ...
      lint: textEditor => {
        ...
        return this.Hack.update(filePath)
        .then(function() {
        ...
        .then(() => {
          return this.Hack.exec(atom.config.get('Atom-Hack.hackExecutablePath'), ['--json'], {cwd: Path.dirname(filePath), stream: 'both'})
        }).then(function(streams) {
          const contents = streams.stdout || streams.stderr || ''
          try {
